...
while True:
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            elif classNames[class_index] == "person":
                cvzone.cornerRect(img, (x1, y1, w, h))
                conf = math.ceil((box.conf[0] * 100)) / 100
                person_focal_length = (person_width * person_d) / w
                #print("person focal: ",person_focal_length)
                person_apparent_width = w  # Apparent width of the cell phone in pixels
                person_distance = ((person_width * 4.56) / person_apparent_width)*92
                cvzone.putTextRect(img, f'person, distance: {int(person_distance)}cm', (max(0, x1), max(35, y1)),scale=1, thickness=1)
